...
impl GcBlock {
    ...
    pub(super) async fn start(&self) -> Result<Guard, BlockingReasons> {
        ...
        if let Some(reasons) = reasons {
            tracing::debug!(summary=?reasons, "[tomo-id-001] gc iteration start blocked");
            Err(reasons)
        } else {
            tracing::debug!("[tomo-id-002] gc iteration start allowed");
            Ok(Guard {
    ...
    pub(crate) fn summary(&self) -> Option<BlockingReasons> {
        ...
        let summary = BlockingReasons::summarize(&g);
        if let Some(reasons) = &summary {
            tracing::debug!(summary=?reasons, "[tomo-id-010] gc blocking summary requested");
        }
        summary
    }
    ...
    pub(crate) async fn insert(
        ...
        let (added, uploaded) = {
            ...
            tracing::debug!(added, inserted=?reason, "[tomo-id-003] scheduling gc block reason insert");
        ...
        uploaded.await?;
        tracing::debug!(inserted=?reason, "[tomo-id-004] gc block reason insert uploaded");

        // ensure that any ongoing gc iteration has completed
        drop(self.blocking.lock().await);
        tracing::debug!(inserted=?reason, "[tomo-id-005] synchronized with ongoing gc iteration after insert");
    ...
    pub(crate) async fn remove(
        ...
        if remaining_blocks > 0 {
            tracing::info!(remaining_blocks, removed=?reason, timeline_id=?timeline.timeline_id, "[tomo-id-006] gc blocking removed, but gc remains blocked");
        } else {
            tracing::info!(timeline_id=?timeline.timeline_id, "[tomo-id-007] gc is now unblocked for the tenant");
        }
    ...
    pub(crate) fn before_delete(&self, timeline_id: &super::TimelineId) {
        ...
        if unblocked {
            tracing::info!(timeline_id=?timeline_id, "[tomo-id-008] gc is now unblocked following deletion");
        }
    }
    ...
    pub(crate) fn set_scanned(&self, scanned: Storage) {
        ...
        if let Some(reasons) = BlockingReasons::clean_and_summarize(g) {
            tracing::info!(summary=?reasons, "[tomo-id-009] initialized with gc blocked");
        }
    }
}
