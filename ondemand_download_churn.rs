...
async fn main_impl(args: Args) -> anyhow::Result<()> {
    ...
    let mgmt_api_client = Arc::new(pageserver_client::mgmt_api::Client::new(
        reqwest::Client::builder()
            ...
            .build()?, // TODO: support ssl_ca_file for https APIs in pagebench.
    ...
    tasks.spawn({
        ...
        async move {
            ...
            loop {
                ...
                info!(
                    "[tomo-id-001] churn_stats",
                    delta_ms = delta.as_millis(),
                    evictions_per_s = format_args!("{evictions_per_s:.2}"),
                    downloads_per_s = format_args!("{downloads_per_s:.2}"),
                    downloads_mibs_per_s = format_args!("{downloads_mibs_per_s:.2}"),
                    timeline_restarts
                );
            }
        }
    ...
    while let Some(res) = tasks.join_next().await {
        if let Err(e) = res {
            tracing::error!("[tomo-id-006] task_join_failed", error = %e);
        }
    }
    let end = Instant::now();
    ...
    info!("[tomo-id-009] benchmark_summary", output_json = %output);
...
async fn timeline_actor(
    ...
    while !token.is_cancelled() {
        debug!(
            "[tomo-id-002] restarting_timeline",
            tenant_id = %timeline.tenant_id,
            timeline_id = %timeline.timeline_id
        );
        let layer_map_info = match mgmt_api_client
            ...
            .await
        {
            ...
            Err(e) => {
                tracing::warn!(
                    "[tomo-id-005] layer_map_info_failed",
                    tenant_id = %timeline.tenant_id,
                    timeline_id = %timeline.timeline_id,
                    error = %e
                );
...
async fn layer_actor(
    ...
    while let Some(_permit) = rx.recv().await {
        ...
        let did_it = match action {
            Action::Evict => {
                let did_it = match mgmt_api_client
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[tomo-id-007] layer_evict_failed",
                            tenant_shard_id = %tenant_shard_id,
                            timeline_id = %timeline_id,
                            layer_file_name = layer.layer_file_name(),
                            error = %e
                        );
                        continue;
                    }
            ...
            Action::OnDemandDownload => {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "[tomo-id-008] layer_ondemand_download_failed",
                            tenant_shard_id = %tenant_shard_id,
                            timeline_id = %timeline_id,
                            layer_file_name = layer.layer_file_name(),
                            error = %e
        ...
        if !did_it {
            debug!(
                "[tomo-id-003] layer_map_out_of_sync",
                tenant_shard_id = %tenant_shard_id,
                timeline_id = %timeline_id,
                layer_file_name = layer.layer_file_name()
            );
        ...
        debug!(
            "[tomo-id-004] layer_action_completed",
            tenant_shard_id = %tenant_shard_id,
            timeline_id = %timeline_id,
            layer_file_name = layer.layer_file_name()
        );
