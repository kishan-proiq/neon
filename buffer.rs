...
impl<A: Alignment> AlignedBuffer<A> {
    ...
    fn as_ptr(&self) -> *const u8 {
        debug_assert!(
            self.range.start <= self.range.end && self.range.end <= self.raw.as_slice().len(),
            "AlignedBuffer range must be within raw buffer bounds"
        );
    ...
    ///
    /// This method panics on invalid ranges. For production-safe callers, prefer `try_slice()`.
    pub fn slice(&self, range: impl RangeBounds<usize>) -> Self {
        self.try_slice(range).expect("out of range")
    }

    /// Returns a slice of self for the index range `[begin..end)` without panicking.
