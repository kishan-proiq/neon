...
impl MockTimeline {
    ...
    pub async fn compact(&mut self) -> anyhow::Result<()> {
        ...
        if let Err(e) = crate::compact_tiered::compact_tiered(
            self,
            self.last_flush_lsn,
        ...
        {
            info!(
                "[tomo-id-005] compaction failed",
                last_flush_lsn = self.last_flush_lsn.0,
                target_file_size = self.target_file_size,
                tiers_per_level = self.tiers_per_level,
                error = %e
            );
            return Err(e);
        }
    ...
    pub async fn compact_if_needed(&mut self) -> anyhow::Result<()> {
        if self.num_l0_flushes - self.last_compact_at_flush >= self.tiers_per_level {
            info!(
                "[tomo-id-006] triggering compaction",
                num_l0_flushes = self.num_l0_flushes,
                last_compact_at_flush = self.last_compact_at_flush,
                tiers_per_level = self.tiers_per_level
            );
            self.compact().await?;
    ...
    pub fn flush_l0(&mut self) {
        ...
        info!(
            "[tomo-id-001] flushed L0 layer",
            layer_id = %new_layer.short_id(),
            file_size = new_layer.file_size,
            lsn_start = new_layer.lsn_range.start.0,
            lsn_end = new_layer.lsn_range.end.0,
            flush_no = self.num_l0_flushes + 1
        );
    ...
    pub fn ingest_uniform(
        ...
        info!(
            "[tomo-id-007] ingest_uniform start",
            num_records,
            record_len = len,
...
impl interface::CompactionJobExecutor for MockTimeline {
    ...
    async fn create_image(
        ...
        info!(
            "[tomo-id-002] created image layer",
            layer_id = %new_layer.short_id(),
            file_size = new_layer.file_size,
            key_start = new_layer.key_range.start,
            key_end = new_layer.key_range.end,
            lsn = new_layer.lsn_range.start.0
        );
    ...
    async fn create_delta(
        ...
        info!(
            "[tomo-id-003] created delta layer",
            layer_id = %new_layer.short_id(),
    ...
    async fn delete_layer(
        ...
        info!(
            "[tomo-id-004] deleting layer",
            layer_id = %layer.short_id(),
            file_size = layer.file_size(),
            deleted_layers_total = self.num_deleted_layers + 1
        );
