        ...
        backoff::retry(
            || async {
                fail::fail_point!("deletion-queue-before-execute", |_| {
                    info!(
                        "[tomo-id-001] Skipping deletion batch execution due to failpoint",
        ...
        while !self.accumulator.is_empty() && !self.cancel.is_cancelled() {
            ...
            match self.remote_delete().await {
                Ok(()) => {
                    ...
                    info!(
                        "[tomo-id-002] Executed deletion batch",
                        batch_size = self.accumulator.len()
                    );
                ...
                Err(e) => {
                    ...
                    warn!(
                        "[tomo-id-003] DeleteObjects request failed; will continue retrying",
                        batch_size = self.accumulator.len(),
                        error = %e
                    );
                    metrics::DELETION_QUEUE
        ...
        loop {
            ...
            let msg = match tokio::time::timeout(AUTOFLUSH_INTERVAL, self.rx.recv()).await {
                ...
                Ok(None) => {
                    ...
                    info!("[tomo-id-004] Shutting down deleter (all senders closed)");
                    return Err(DeletionQueueError::ShuttingDown);
                }
                Err(_) => {
