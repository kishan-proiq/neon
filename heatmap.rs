...
impl HeatMapTenant {
    pub(crate) fn into_timelines_index(self) -> HashMap<TimelineId, HeatMapTimeline> {
        // Debug-only safety: duplicate timeline IDs would silently overwrite in HashMap::collect.
        // Keeping this as a debug assertion avoids changing release behavior.
        debug_assert_eq!(
            self.timelines.len(),
            self.timelines
                .iter()
                ...
                .len(),
            "duplicate timeline_id in heatmap"
        );
    ...
    pub(crate) fn get_stats(&self) -> HeatMapStats {
        ...
        for timeline in &self.timelines {
            for layer in timeline.hot_layers() {
                stats.layers += 1;
                // Defensive: avoid overflow if metadata is corrupted/untrusted.
                stats.bytes = stats.bytes.saturating_add(layer.metadata.file_size);
            }
        }
