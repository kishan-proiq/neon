...
async fn main_impl(
        ...
        js.spawn({
            ...
            async move {
                let info = mgmt_api_client
                    ...
                    .await
                    .map_err(|e| {
                        info!("[tomo-id-001] timeline_info failed during discovery", tenant_id = %timeline.tenant_id, timeline_id = %timeline.timeline_id, error = %e);
                        e
                    })?;
                anyhow::Ok(Target {
                    timeline,
    ...
    while let Some(res) = js.join_next().await {
        match res {
            Ok(Ok(target)) => all_targets.push(target),
            Ok(Err(e)) => return Err(e),
            Err(e) => return Err(anyhow!("[tomo-id-002] discovery task join failed: {e}")),
        }
    }
    ...
    tokio::spawn({
        ...
        async move {
            ...
            loop {
                ...
                info!(
                    "[tomo-id-003] RPS tick",
                    rps = (completed_requests as f64 / elapsed.as_secs_f64())
                );
            }
        }
    });
    ...
    let work_sender = async move {
        ...
        loop {
            let (timeline, work) = {
                ...
                let Some(target) = all_targets.choose(&mut rng) else {
                    info!("[tomo-id-004] no targets available; stopping work sender");
                    return;
                };
                let lsn = target.lsn_range.clone().map(|r| rng.random_range(r));
                (target.timeline, Work { lsn })
            };
            let Some(sender) = work_senders.get(&timeline) else {
                info!("[tomo-id-005] missing sender for timeline; stopping work sender", tenant_id = %timeline.tenant_id, timeline_id = %timeline.timeline_id);
                return;
            };
            // TODO: what if this blocks?
            if sender.send(work).await.is_err() {
                info!("[tomo-id-006] worker channel closed; stopping work sender", tenant_id = %timeline.tenant_id, timeline_id = %timeline.timeline_id);
                return;
            }
        }
    };
    ...
    info!("[tomo-id-010] benchmark summary", summary_json = %output);
...
async fn run_worker(
    ...
    while let Some(Work { lsn }) = work.recv().await {
        ...
        let stream = match client.basebackup(lsn).await {
            Ok(stream) => stream,
            Err(e) => {
                info!("[tomo-id-007] basebackup request failed", error = %e, lsn = ?lsn);
                continue;
            }
        };

        let size = match futures::io::copy(stream.compat(), &mut tokio::io::sink().compat_write()).await {
            Ok(size) => size,
            Err(e) => {
                info!("[tomo-id-008] basebackup stream copy failed", error = %e, lsn = ?lsn);
                continue;
            }
        };
        info!("[tomo-id-009] basebackup completed", size_bytes = size, lsn = ?lsn);
        let elapsed = start.elapsed();
        live_stats.inc();
        STATS.with(|stats| {
