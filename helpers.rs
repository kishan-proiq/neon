...
{
    let _span = tracing::info_span!("merge_delta_keys_buffered", layers = layers.len()).entered();
    tracing::debug!("[tomo-id-001] starting buffered delta key merge");
    ...
    for (idx, l) in layers.iter().enumerate() {
        ...
        let loaded = load_future.await?;
        tracing::debug!("[tomo-id-002] loaded delta keys for layer", layer_index = idx, loaded_keys = loaded.len());
        keys.extend(loaded.into_iter());
    }
    tracing::debug!("[tomo-id-003] sorting merged delta keys", total_keys = keys.len());
            ...
            if let Some(mut top) = this.heap.peek_mut() {
                match top.deref_mut() {
                    ...
                    LazyLoadLayer::Loaded(entries) => {
                        let Some(result) = entries.pop_front() else {
                            tracing::warn!("[tomo-id-004] loaded layer had no entries when polled");
    ...
    D: CompactionDeltaEntry<'a, K>,
{
    ...
    async_stream::try_stream! {
        ...
        let mut emitted_keys: u64 = 0;
        let mut emitted_values: u64 = 0;
        ...
        if let Some(first) = input.next().await {
            ...
            while let Some(this) = input.next().await {
                ...
                if this.key() == accum.key {
                    ...
                    debug_assert!(last_key <= accum.key, "last_key={last_key} <= accum.key={}", accum.key);
                    last_key = accum.key;
                    emitted_keys += 1;
                    emitted_values += accum.num_values;
                    yield accum;
                    part_size = this.size();
                    accum = KeySize {
                        key: this.key(),
        ...
        tracing::debug!("[tomo-id-005] accum_key_values completed", target_size, emitted_keys, emitted_values);
    }
}
