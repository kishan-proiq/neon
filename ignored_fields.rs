...
pub fn find(user_specified: toml_edit::DocumentMut, reserialized: toml_edit::DocumentMut) -> Paths {
    ...
    tracing::debug!(
        "[tomo-id-001] computed ignored config fields",
        ignored_fields_count = paths.len(),
        elapsed_ms = _t0.elapsed().as_millis()
    );
...
impl<'a> PathsVisitor<'a> {
    ...
    fn visit_table_like(&mut self, table_like: &dyn toml_edit::TableLike) {
        for (entry, item) in table_like.iter() {
            ...
            debug_assert!(
                !self.stack.is_empty(),
                "[tomo-id-002] PathsVisitor stack underflow"
            );
            self.stack.pop();
        }
    }
    ...
    fn visit_item(&mut self, item: &toml_edit::Item) {
        // Overload protection: prevent pathological TOML from causing unbounded traversal.
        // Note: this intentionally truncates traversal on extreme inputs to keep the pageserver alive.
        const MAX_VISITED_NODES: usize = 200_000;
        if self.out.len() >= MAX_VISITED_NODES {
            tracing::warn!(
                "[tomo-id-003] ignored_fields traversal truncated due to node limit",
    ...
    fn visit_value(&mut self, value: &toml_edit::Value) {
        // Overload protection: prevent extremely deep nesting from causing excessive recursion.
        const MAX_STACK_DEPTH: usize = 256;
        if self.stack.len() > MAX_STACK_DEPTH {
            tracing::warn!(
                "[tomo-id-004] ignored_fields traversal truncated due to depth limit",
...
pub(crate) mod tests {
    ...
    fn test_impl(original: &str, parsed: &str, expect: &[&str]) {
        let original: toml_edit::DocumentMut = original
            .parse()
            .expect("[tomo-id-005] parse original config");
        let parsed: toml_edit::DocumentMut = parsed
            .parse()
            .expect("[tomo-id-006] parse re-serialized config");
