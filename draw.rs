...
impl<T: Ord + Copy> CoordinateMap<T> {
    fn new(coords: Vec<T>, stretch: f32) -> Self {
        ...
        map.extend(set.iter().enumerate().map(|(i, e)| (*e, i)));
...
pub fn draw_history<W: std::io::Write>(history: &[LayerTraceEvent], mut output: W) -> Result<()> {
    tracing::debug!("[tomo-id-001] draw_history_start", history_len = history.len());
    ...
    let last_time_rel = history
        ...
        .time_rel;
    tracing::debug!("[tomo-id-002] draw_history_last_time", last_time_rel = last_time_rel);

    // Collect all coordinates
    // Defensive cap: prevent OOM when history is unexpectedly huge.
    const MAX_EVENTS: usize = 1_000_000;
    if history.len() > MAX_EVENTS {
        tracing::warn!("[tomo-id-005] draw_history_input_too_large", history_len = history.len(), max_events = MAX_EVENTS);
        return Err(anyhow::anyhow!("history too large: {} (max {})", history.len(), MAX_EVENTS));
    }
    ...
    for f in files_sorted {
        ...
        if key_start >= key_end {
            tracing::error!("[tomo-id-003] invalid_key_range", filename = %f.filename, key_start = key_start, key_end = key_end);
            return Err(anyhow::anyhow!("Invalid key range {key_start}-{key_end} for {}", f.filename));
        }
        ...
        match f.lsn_range.start.cmp(&f.lsn_range.end) {
            ...
            Ordering::Greater => {
                tracing::error!("[tomo-id-004] invalid_lsn_range", filename = %f.filename, lsn_start = lsn_start, lsn_end = lsn_end);
                return Err(anyhow::anyhow!("Invalid lsn range {lsn_start}-{lsn_end} for {}", f.filename));
            }
        }
        files_seen.insert(f);
    }
