...
fn parse_filename(name: &str) -> (Range<Key>, Range<Lsn>) {
    ...
    let keys_part = split.first().copied().unwrap_or("");
    let lsns_part = split.get(1).copied().unwrap_or("");
    ...
    let lsns = lsns_part.split('.').collect::<Vec<&str>>()[0];
    ...
    if lsns.last().map(|s| s.len()) == Some(8) {
        lsns.pop();
    }

    // Handle version number `-v1` part
    if lsns.last().map(|s| s.starts_with('v')) == Some(true) {
        lsns.pop();
    }
    ...
    let keys = Key::from_hex(keys.get(0).copied().unwrap_or(""))
        .context("[tomo-id-006] parse key start")?
        ..Key::from_hex(keys.get(1).copied().unwrap_or(""))
            .context("[tomo-id-007] parse key end")?;
    let lsns = Lsn::from_hex(lsns.get(0).copied().unwrap_or(""))
        .context("[tomo-id-008] parse lsn start")?
        ..Lsn::from_hex(lsns.get(1).copied().unwrap_or(""))
            .context("[tomo-id-009] parse lsn end")?;
    (keys, lsns)
}
...
pub fn main() -> Result<()> {
    eprintln!("[tomo-id-001] draw-timeline start version={GIT_VERSION}");
    ...
    // Guardrails: avoid accidental OOM when stdin is unexpectedly large.
    const MAX_STDIN_LINES: usize = 1_000_000;
    const MAX_LINE_BYTES: usize = 16 * 1024;

    for (lineno, line) in stdin.lock().lines().enumerate() {
        let lineno = lineno + 1;
        if lineno > MAX_STDIN_LINES {
            anyhow::bail!("[tomo-id-010] stdin exceeded MAX_STDIN_LINES={MAX_STDIN_LINES}");
        }

        let line = line.with_context(|| format!("[tomo-id-002] failed to read stdin line {lineno}"))?;
        if line.len() > MAX_LINE_BYTES {
            anyhow::bail!("[tomo-id-011] stdin line {lineno} exceeded MAX_LINE_BYTES={MAX_LINE_BYTES}");
        }
        ...
        let line = PathBuf::from_str(&line)
            .with_context(|| format!("[tomo-id-003] invalid path on stdin line {lineno}: {line:?}"))?;
        let filename = line
            .file_name()
            .with_context(|| format!("[tomo-id-004] missing filename component on stdin line {lineno}: {line:?}"))?;
        let filename = filename
            .to_str()
            .with_context(|| format!("[tomo-id-005] non-utf8 filename on stdin line {lineno}: {filename:?}"))?;
    ...
    // Draw
    let stretch: f32 = std::env::var("NEON_PAGECTL_DRAW_TIMELINE_STRETCH")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(3.0); // Stretch out vertically for better visibility
    println!(
        "{}",
        BeginSvg {
    ...
    } in &files
    {
        ...
        if key_start >= key_end {
            anyhow::bail!("[tomo-id-012] invalid key range key_start={key_start} key_end={key_end} layer={filename}");
        }

        let lsn_start = *lsn_map.get(&lsnr.start).with_context(|| {
            format!("[tomo-id-013] missing lsn_start in compression map layer={filename} lsn={}", lsnr.start)
        })?;
        let lsn_end = *lsn_map.get(&lsnr.end).with_context(|| {
            format!("[tomo-id-014] missing lsn_end in compression map layer={filename} lsn={}", lsnr.end)
        })?;
    ...
    for (lsn, kind) in lines {
        let lsn_start = *lsn_map.get(&lsn).with_context(|| {
            format!("[tomo-id-015] missing LSN in compression map for line marker lsn={lsn}")
        })?;
    ...
    eprintln!("[tomo-id-016] num_images: {num_images}");
    eprintln!("[tomo-id-017] num_deltas: {num_deltas}");
    eprintln!("[tomo-id-018] draw-timeline complete layers_total={} markers_total={}", files.len(), lsn_map.len());
