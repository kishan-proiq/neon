...
impl<'a> FilterIterator<'a> {
    pub fn create(
        ...
        let mut retain_key_filters = Vec::with_capacity(
            dense_keyspace.ranges.len() + sparse_keyspace.0.ranges.len(),
        );
        ...
        for (idx, window) in retain_key_filters.windows(2).enumerate() {
            if window[0].end > window[1].start {
                bail!(
                    "Key filters are overlapping at idx={} (prev_end={:?} > next_start={:?}); prev={:?} next={:?}",
                    idx,
                    window[0].end,
                    window[1].start,
                    window[0],
    ...
    async fn next_inner<R: MergeIteratorItem>(&mut self) -> anyhow::Result<Option<R>> {
        while let Some(item) = self.inner.next_inner::<R>().await? {
            let (key, _lsn, _value) = item.key_lsn_value();

            while self.current_filter_idx < self.retain_key_filters.len()
                && key >= self.retain_key_filters[self.current_filter_idx].end
            {
            ...
            let current_filter = &self.retain_key_filters[self.current_filter_idx];
            if current_filter.contains(&key) {
                // [filter region]    [filter region]     [filter region]
                //                                              ^ item
                //                                        ^ current filter
                return Ok(Some(item));
