...
async fn main_impl(
    ...
    let cache = args
        ...
        .map(|keyspace_cache_file| {
            ...
            info!("[tomo-id-001] keyspace cache file matches tag: {tag_ok}");
            anyhow::Ok(if tag_ok { Some(cache.data) } else { None })
        })
        .transpose()?
        .flatten();
    let all_ranges: Vec<KeyRange> = if let Some(cached) = cache {
        info!("[tomo-id-002] using keyspace cache file");
        cached
    } else {
        ...
        info!("[tomo-id-009] filter duration: {}", total_filter_duration.as_secs_f64());
        if let Some(cachefile) = args.keyspace_cache.as_ref() {
            ...
            info!("[tomo-id-010] successfully wrote keyspace cache file");
        }
        all_ranges
    };
    ...
    info!("[tomo-id-004] spawning workers");
    ...
    info!("[tomo-id-005] waiting for everything to become ready");
    shared_state.start_work_barrier.wait().await;
    info!("[tomo-id-006] work started");
    if let Some(runtime) = args.runtime {
        tokio::time::sleep(runtime.into()).await;
        info!("[tomo-id-007] runtime over, signalling cancellation");
        cancel.cancel();
        workers.await;
        info!("[tomo-id-008] work sender exited");
    } else {
        workers.await;
